...
2. Resolves the commit range (`from-tag` to `to-tag`), using the latest published GitHub
   ...
   (or full history when none exists); without a local repository and without
   `tag-prefix`, `github-native` leaves previous-tag selection to GitHub
...
| `from-tag` | — | latest published ancestor | Start of commit range for notes (exclusive); constrained by `tag-prefix` and local ancestry; with `github-native`, no local repository, and no `tag-prefix`, GitHub selects the previous tag |
...
`previous_tag_name`. When `from-tag` and `tag-prefix` are omitted and no local Git
repository is available, the action omits `previous_tag_name` so GitHub selects the
previous tag. Set `from-tag` explicitly when using `tag-prefix` without a local
repository. Because GitHub generates notes for the release tag and does not support
path filtering, `to-tag` must equal `tag` and `path-filter` cannot be used with this
format.
